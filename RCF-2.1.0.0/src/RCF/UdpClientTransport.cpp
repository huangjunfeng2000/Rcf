...
//-------------------------------------------------------
	MQClientTransport::MQClientTransport() :
		//mAsync(),
		mSock(-1)
	{

	}
	MQClientTransport::MQClientTransport(const IpAddress & ipAddress) :
	mDestIp(ipAddress),
		mAsync(),
		mSock(-1)
	{
	...
	MQClientTransport::MQClientTransport(const MQClientTransport &rhs) :
	ClientTransport(rhs),
		mDestIp(rhs.mDestIp),
		mAsync(),
		mSock(-1)
	...
	TransportType MQClientTransport::getTransportType()
	{
		return Tt_ZeroMQ;
	}

	ClientTransportAutoPtr MQClientTransport::clone() const
	{
		return ClientTransportAutoPtr( new MQClientTransport(*this));
	}

	EndpointPtr MQClientTransport::getEndpointPtr() const
	{
		return EndpointPtr( new MQEndpoint(/*mDestIp*/) );
	}

	void MQClientTransport::setTimer(
		boost::uint32_t timeoutMs,
		ClientTransportCallback *pClientStub)
	{
		RCF_UNUSED_VARIABLE(timeoutMs);
	...
	void MQClientTransport::connect(
		ClientTransportCallback &clientStub, 
		unsigned int timeoutMs)
	{
//		RCF_LOG_4()(mSock)(mDestIp.string()) << "MQClientTransport - creating socket.";
//
//		RCF_UNUSED_VARIABLE(timeoutMs);
//
//		RCF_ASSERT(!mAsync);
...
//
	...
	int MQClientTransport::send(
		ClientTransportCallback &clientStub, 
		const std::vector<ByteBuffer> &data,
		unsigned int timeoutMs)
	{
		if (mCallBackSend && !mCallBackSend.empty())
		{
			mCallBackSend(data[0].getPtr(), data[0].getLength());
		}
		//if (FILE *fp = fopen("c:\\send.txt", "wb"))
		//{
		//	fwrite(data[0].getPtr(), data[0].getLength(), 1, fp);
		//	fclose(fp);
		//}
		//RCF_LOG_4()(mSock)(mDestIp.string()) << "MQClientTransport - sending data on socket.";
	...
	void MQClientTransport::SetCallBackFunc(CallBack_SendCommand sendFunc, CallBack_GetResult recvFunc)
	{
		mCallBackSend = sendFunc;
		mCallBackRecv = recvFunc;
	}
	int MQClientTransport::receive(
		ClientTransportCallback &clientStub, 
		ByteBuffer &byteBuffer,
		unsigned int totalTimeoutMs)
	{
		if (mCallBackRecv && !mCallBackRecv.empty())
		{
			size_t iSize = 0;
			void *pReadStr = mCallBackRecv(iSize);
			//const std::vector<unsigned char> &vecResult = mCallBackSend();
			if (mReadVecPtr.get() == NULL || !mReadVecPtr.unique())
			{
				mReadVecPtr.reset( new ReallocBuffer());
			}
			//size_t iSize = vecResult.size();
			if (iSize > 4)
			{
				ReallocBuffer &buffer = *mReadVecPtr;
				buffer.resize(iSize);
				memcpy(&buffer[0], pReadStr, iSize);
		...
		RCF_THROW( Exception( _RcfError_ClientReadFail() ) )(0)(0);
		...
		//}
		//// try to receive a UDP message from server, within the current timeout
		//RCF_LOG_4()(mSock)(mDestIp.string())(totalTimeoutMs) << "MQClientTransport - receiving data from socket.";
...
return 1;
	...
	void MQClientTransport::setTransportFilters(
		const std::vector<FilterPtr> &filters)
	{
		if (!filters.empty())
		{
	...
	void MQClientTransport::getTransportFilters(
		std::vector<FilterPtr> &filters)
	{
		RCF_UNUSED_VARIABLE(filters);
	}

	int MQClientTransport::getNativeHandle() const
	{
		return mSock;
	}
