...
//--------------------------------------------------------
	MQServerTransport::MQServerTransport() :
	mpRcfServer(),
		//mIpAddress(ipAddress),
		//mMulticastIpAddress(multicastIpAddress),
		mFd(-1),
	...
	TransportType MQServerTransport::getTransportType()
	{
		return Tt_ZeroMQ;
	}

	MQServerTransport & MQServerTransport::enableSharedAddressBinding()
	{
		mEnableSharedAddressBinding = true;
		return *this;
	}

	ServerTransportPtr MQServerTransport::clone()
	{
		return ServerTransportPtr( new MQServerTransport(
			mIpAddress, 
			mMulticastIpAddress) );
	}

	void MQServerTransport::setSessionManager(RcfServer & rcfServer)
	{
		mpRcfServer = &rcfServer;
	}

	RcfServer & MQServerTransport::getSessionManager()
	{
		RCF_ASSERT(mpRcfServer);
		return *mpRcfServer;
	}

	int MQServerTransport::getPort() const
	{
		return mIpAddress.getPort();
	}

	void MQServerTransport::open()
	{
//		RCF_LOG_4()(mIpAddress.string()) << "MQServerTransport - creating server socket.";
//
//		int mPort = mIpAddress.getPort();
//
//		// create and bind a socket for receiving UDP messages
...
//			}
//
//			RCF_LOG_2() << "MQServerTransport - listening on port " << mPort << ".";
//		}
	}

	void MQServerTransport::close()
	{
		//if (mFd != -1)
		//{
		//	int ret = Platform::OS::BsdSockets::closesocket(mFd);
	...
	void MQServerTransport::cycle(int timeoutMs)
	{
		...
		//if (ret == 1)
		//{
		//	NetworkSessionPtr networkSessionPtr = getTlsMQNetworkSessionPtr();
		//	if (networkSessionPtr.get() == NULL)
		//	{
		//		networkSessionPtr = NetworkSessionPtr(new NetworkSession(*this));
		//		networkSessionPtr->mRcfSessionPtr = getSessionManager().createSession();
		//		networkSessionPtr->mRcfSessionPtr->setNetworkSession(*networkSessionPtr);
		//		setTlsMQNetworkSessionPtr(networkSessionPtr);
		//	}
		...
		//else if (ret == 0)
		//{
		//	//RCF_LOG_4()(mFd)(mPort)(timeoutMs) << "MQServerTransport - no messages received within polling interval.";
		//}
		//else if (ret == -1)
		//{
		//	Exception e(
	...
	ReallocBufferPtr MQNetworkSession::GetResult()
	{
		return mWriteVecPtr;
	}
	//gdatanet send
	void MQNetworkSession::postWrite(
		std::vector<ByteBuffer> &byteBuffers)
	{
		// prepend data length and send the data
	...
	void MQNetworkSession::postRead()
	{
	}

	void MQServerTransport::cycleTransportAndServer(int timeoutMs)
	{
		cycle(timeoutMs);
	}

	void MQServerTransport::onServiceAdded(RcfServer &server)
	{
		setSessionManager(server);
		...
		mTaskEntries.push_back(
			TaskEntry(
			boost::bind(
			&MQServerTransport::cycleTransportAndServer,
			this,
			_1),
			StopFunctor(),
			"RCF UDP server"));
	...
	void MQServerTransport::onServiceRemoved(RcfServer &)
	{}

	MQNetworkSession::MQNetworkSession(MQServerTransport & transport) :
			mTransport(transport)
	{

	}

	void MQServerTransport::onServerStart(RcfServer &)
	{
		open();
	}

	void MQServerTransport::onServerStop(RcfServer &)
	{
		close();
	}

	void MQServerTransport::tryReadMessage(NetworkSessionPtr networkSessionPtr)
	{
		// Try to read a message from the UDP socket.
		RCF_UNUSED_VARIABLE(networkSessionPtr);
		//int err = 0;
	...
	const RemoteAddress &MQNetworkSession::getRemoteAddress() const
	{
		return mRemoteAddress;
	}

	ServerTransport & MQNetworkSession::getServerTransport()
	{
		return mTransport;
	}

	const RemoteAddress & MQNetworkSession::getRemoteAddress()
	{
		return mRemoteAddress;
	}

	void MQNetworkSession::setTransportFilters(const std::vector<FilterPtr> &filters)
	{
		RCF_UNUSED_VARIABLE(filters);
		RCF_ASSERT(0);
	}

	void MQNetworkSession::getTransportFilters(std::vector<FilterPtr> &filters)
	{
		RCF_UNUSED_VARIABLE(filters);
		RCF_ASSERT(0);
	}
	void MQNetworkSession::SetMessage(void *pStr, int iSize)
	{
		ReallocBufferPtr &readVecPtr =
			mReadVecPtr;
		...
		ReallocBuffer &buffer = *readVecPtr;
		buffer.resize(iSize);
		memcpy(buffer.getPtr(), pStr, iSize);
	}

	//gdatanet recv
	ByteBuffer MQNetworkSession::getReadByteBuffer()
	{
		ReallocBufferPtr &readVecPtr =
			mReadVecPtr;

		assert(readVecPtr.get() != NULL);
		//if (readVecPtr.get() == NULL || !readVecPtr.unique())
		//{
		//	readVecPtr.reset(new ReallocBuffer());
		//}
	...
	void MQNetworkSession::postClose()
	{
	}

	int MQNetworkSession::getNativeHandle() const
	{
		return mTransport.mFd;
	}

	bool MQNetworkSession::isConnected()
	{
		return true;
	}
