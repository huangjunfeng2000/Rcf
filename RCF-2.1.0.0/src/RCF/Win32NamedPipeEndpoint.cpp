...
namespace RCF {
	...
	MQEndpoint::MQEndpoint(const tstring & pipeName) :
	mPipeName(pipeName)
	{}

#if RCF_FEATURE_NAMEDPIPE==1

	ServerTransportAutoPtr MQEndpoint::createServerTransport() const
	{
		return ServerTransportAutoPtr(
			new MQServerTransport(/*mPipeName*/));
	}

	ClientTransportAutoPtr MQEndpoint::createClientTransport() const
	{            
		return ClientTransportAutoPtr(
			new MQClientTransport(/*mPipeName*/));
	}
	...
	EndpointPtr MQEndpoint::clone() const
	{
		return EndpointPtr( new MQEndpoint(*this) );
	}

	std::string MQEndpoint::asString() const
	{
		MemOstream os;
		os << "pipe://" << RCF::toAstring(mPipeName);
		return os.string();
