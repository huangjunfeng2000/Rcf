...
namespace RCF {
    ...
    void RcfSession::onReadCompleted()
    {
        ...
        if (!mStopCallInProgress)
        {
            ...
            if (mpNetworkSession &&    getTransportType() == Tt_Win32NamedPipe 
			...
			if (mpNetworkSession)
				readByteBuffer = getNetworkSession().getReadByteBuffer();
			else
			{
			...
			//if (FILE *fp = fopen("c:\\onReadCompleted.txt", "wb"))
			//{
			//	fwrite(readByteBuffer.getPtr(), readByteBuffer.getLength(), 1, fp);
			//	fclose(fp);
			//}
